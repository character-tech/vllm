...
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning) # disable annoying security warning
...
def format_user_chats(prompt_data):
...
def extract_prompt_response_from_generation(generation):
    """Extract prompt and response from a single generation"""
    try:
        prompt_data = generation["payload"]["prompt_data"]
        
        # Format the chat context as the prompt
...
def load_static_generations(filename=STATIC_PATH, limit=None, random_selection=False):
    """Load generations from static file"""
...
def use_replay_chat(limit=10, random_selection=False):
    print(f"Loading chat data from static source...")
    prompt_response_pairs = []

    try:
        generations = load_static_generations(limit=limit, random_selection=random_selection)
        print(f"Loaded {len(generations)} generations from static file")
    except Exception as e:
        print(f"Error loading static file: {e}")
        print("Note: You may need access to the GCS bucket or a local copy of the data")
...
@DeprecationWarning
def generate_logprobs_vllm(): 
    from vllm import LLM, SamplingParams
    checkpoint_dir = '/data/rohin/ckpts/roar_sft_adventure_safe.05-01-22-00-25.uc33.classi_only.tp1.minimal/'
    # os.environ["VLLM_LOGITS_DIR"] = "/data/rohin/logits"

    print(f"logits dir: {os.environ['VLLM_LOGITS_DIR']}")
    ...
    # import code; code.interact(local=dict(globals(), **locals()))
    for prompt in ["What is AGI?", "What is the capital of France?"]:
        # prompt = "<|beginningofdialog|>" + prompt
...
def logits_hash(logits):
    ...
    return hashlib.sha256(rounded_logits.to(torch.float32).numpy().tobytes()).hexdigest()[:10]
...
def calculate_statistics(vllm_logits_file, roller_logits_file, eos_tok_id):
    ...
    def smart_clamp(logits, max_range=100, clamp_range=50):
        if logits.max() - logits.min() > max_range:
            ...
            return torch.clamp(logits, min = center-clamp_range, max = center+clamp_range)
        return logits
    ...
    # print(f"VLLM probs: min={vllm_probs.min():.2e}, max={vllm_probs.max():.2e} sum={vllm_probs.sum():.2e}")
    # print(f"Roller probs: min={roller_probs.min():.2e}, max={roller_probs.max():.2e} sum={roller_probs.sum():.2e}")
    ...
    kl_div = torch.nn.functional.kl_div(vllm_log_probs, roller_log_probs, 
                                       reduction="batchmean", log_target=True)
    rev_kl_div = torch.nn.functional.kl_div(roller_log_probs, vllm_log_probs,
                                       reduction="batchmean", log_target=True)
    ...
    tv_distance = 0.5 * torch.sum(torch.abs(vllm_probs - roller_probs), dim=-1).mean()
    
    hellinger = torch.sqrt(0.5 * torch.sum((torch.sqrt(vllm_probs) - 
                                          torch.sqrt(roller_probs))**2, dim=-1)).mean()
    
    top_k = 10
    vllm_topk_mass = torch.topk(vllm_probs, top_k, dim=-1)[0].sum(dim=-1).mean()
    roller_topk_mass = torch.topk(roller_probs, top_k, dim=-1)[0].sum(dim=-1).mean()
    ...
    log_rev_kl = torch.log10(torch.clamp(
        torch.nn.functional.kl_div(roller_log_probs, vllm_log_probs, 
                                  reduction="batchmean", log_target=True), 
        min=1e-10))
...
def print_statistics(roller_outer_dir, vllm_outer_dir, prompt_dir, new_files):
    ...
    for file in new_files:
        ...
        computed_stats = calculate_statistics(vllm_logits_file, roller_logits_file, eos_id)
    ...
    print(f"CUMULATIVE_STATS: VLLM EV message length: {vllm_ev_message_length}")
    print(f"CUMULATIVE_STATS: Roller EV message length: {roller_ev_message_length}")
    print(f"CUMULATIVE_STATS: VLLM EV message length / Roller EV message length: {vllm_ev_message_length / roller_ev_message_length}")
    print(f"CUMULATIVE_STATS: P(VLLM still generating): {prob_vllm_still_generating}")
    print(f"CUMULATIVE_STATS: P(Roller still generating): {prob_roller_still_generating}")
    for k, v in cumulative_stats.items():
        print(f"CUMULATIVE_STATS: {k}: {v / idx}")

def load_tokenizer():
    CHECKPOINT_DIR = os.getenv("CHECKPOINT_DIR", "/data/rohin/ckpts/roar_sft_adventure_safe.05-01-22-00-25.uc33.classi_only.tp1.minimal/")
...
def execute_vllm(prompt):
    ...
    data = {
        "prompt": f"<|beginningofdialog|>{prompt}"
    }
...
def get_files_and_hashes(dir):
    return [(file, os.path.getmtime(os.path.join(dir, file))) for file in os.listdir(dir)]
...
def process_prod_response_pair(prompt, response):
    ...
    before_files = get_files_and_hashes(os.path.join(roller_outer_dir, prompt_dir)) # hack to allow for collisions in prompt directories
    # print(f"Before files: {before_files}")
    execute_models(prompt, response)
    after_files = get_files_and_hashes(os.path.join(roller_outer_dir, prompt_dir))
